...
class Blacklist
{
    ...
    public function add($payload)
    {
        ...
        $this->set("JWT:".$uid.":".$this->getKey($payload), $this->getGraceTimestamp(), $this->getSecondsUntilExpired($payload));
    ...
    {
        $uid = $payload['uid']->getValue();
        return $this->get("JWT:".$uid.":".$this->getKey($payload)) ? true : false;
    }

    public function hasGracePeriod($payload)
    {
        $uid = $payload['uid']->getValue();
        $val = (int) $this->get("JWT:".$uid.":".$this->getKey($payload));
